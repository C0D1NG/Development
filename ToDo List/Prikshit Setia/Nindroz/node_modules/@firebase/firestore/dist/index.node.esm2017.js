...
class SyncEngineImpl {
    ...
    async listen(query) {
        ...
        if (queryView) {
            ...
            const targetData = await allocateTarget(this.localStore, queryToTarget(query));
    ...
    async initializeViewAndComputeSnapshot(query, targetId, current) {
        const queryResult = await executeQuery(this.localStore, query, 
    ...
    async unlisten(query) {
        ...
        if (this.isPrimaryClient) {
            ...
            if (!targetRemainsActive) {
                await releaseTarget(this.localStore, queryView.targetId, 
    ...
    async applyRemoteEvent(remoteEvent) {
        ...
        try {
            const changes = await applyRemoteEventToLocalCache(this.localStore, remoteEvent);
    ...
    async rejectListen(targetId, err) {
        ...
        if (limboKey) {
            ...
            await releaseTarget(this.localStore, targetId, 
            /* keepPersistedTargetData */ false)
    ...
    async applySuccessfulWrite(mutationBatchResult) {
        ...
        try {
            const changes = await acknowledgeBatch(this.localStore, mutationBatchResult);
    ...
    async registerPendingWritesCallback(callback) {
        ...
        try {
            const highestBatchId = await getHighestUnacknowledgedBatchId(this.localStore);
    ...
    async emitNewSnapsAndNotifyLocalStore(changes, remoteEvent) {
        ...
        this.queryViewsByQuery.forEach((_, queryView) => {
            queriesProcessed.push(Promise.resolve()
                ...
                return executeQuery(this.localStore, queryView.query, 
...
class RemoteStore {
    ...
    async onWatchStreamChange(watchChange, snapshotVersion) {
        ...
        if (!snapshotVersion.isEqual(SnapshotVersion.min())) {
            try {
                const lastRemoteSnapshotVersion = await getLastRemoteSnapshotVersion(this.localStore);
    ...
    async fillWritePipeline() {
        ...
        while (this.canAddToWritePipeline()) {
            try {
                const batch = await nextMutationBatch(this.localStore, lastBatchIdRetrieved);
                if (batch === null) {
                    if (this.writePipeline.length === 0) {
